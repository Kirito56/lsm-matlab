...
* Copyright (c) 1993-2001 Lester Ingber.  All Rights Reserved.
* The LICENSE file must be included with ASA code.
***********************************************************************/

#define ASA_ID \
"/* $Id: asa.c,v 1.1 2003/02/12 12:01:09 haeusler Exp $ */"
...
#endif /* HAVE_ANSI */
{
...
#if MULTI_MIN
  int multi_index;
  int multi_sort[MULTI_NUMBER + 1];
  int multi_test, multi_test_cmp, multi_test_dim;
  double *multi_params[MULTI_NUMBER + 1];
#endif /* MULTI_MIN */
  ...
  if ((number_generated = (LONG_INT *) calloc (1, sizeof (LONG_INT))) == NULL) {
  ...
  if ((number_accepted = (LONG_INT *) calloc (1, sizeof (LONG_INT))) == NULL) {
  ...
  *curvature_flag = FALSE;
  ...
  if ((index_parameter_generations =
       (LONG_INT *) calloc (*number_parameters, sizeof (LONG_INT))) == NULL) {
...
#if MULTI_MIN
  for (multi_index = 0; multi_index <= MULTI_NUMBER; ++multi_index) {
  ...
  *best_number_generated_saved =
...
#if MULTI_MIN
  multi_sort[MULTI_NUMBER] = MULTI_NUMBER;
  multi_cost[MULTI_NUMBER] = current_generated_state->cost;
  VFOR (index_v) {
    multi_params[MULTI_NUMBER][index_v] =
      current_generated_state->parameter[index_v];
  }
  for (multi_index = 0; multi_index < MULTI_NUMBER; ++multi_index) {
  ...
  while (((*number_accepted <= OPTIONS->Limit_Acceptances)
    ...
    if (asa_read == TRUE && OPTIONS->Asa_Recursive_Level == asa_recursive_max) {
      ...
      for (multi_index = 0; multi_index < MULTI_NUMBER; ++multi_index) {
        fread (&(OPTIONS->Multi_Cost[multi_index]),
               sizeof (double), 1, ptr_save);
        fread (&(OPTIONS->Multi_Params[multi_index]),
               sizeof (double), *number_parameters, ptr_save);
      }
#endif
#if ASA_PARALLEL
      ...
      *accepted_to_generated_ratio =
      ...
      if (((OPTIONS->Multi_Specify == 0)
        ...
        if (multi_test == 0) {
          ...
          qsort (multi_sort, MULTI_NUMBER + 1, sizeof (int), multi_compare);
          for (multi_index = 0; multi_index < MULTI_NUMBER; ++multi_index) {
...
*number_accepted=%d  *number_generated=%d\n", G_FIELD, G_PRECISION, best_generated_state->cost,
    ...
    if (*recent_number_acceptances == 0
      ...
      for (multi_index = 0; multi_index < MULTI_NUMBER; ++multi_index) {
        fwrite (&(OPTIONS->Multi_Cost[multi_index]),
                sizeof (double), 1, ptr_save);
        fwrite (&(OPTIONS->Multi_Params[multi_index]),
                sizeof (double), *number_parameters, ptr_save);
      }
#endif
#if ASA_PARALLEL
...
#endif
{
  ...
  if ((curvature_flag = (int *) calloc (1, sizeof (int))) == NULL) {
    strcpy (exit_msg, "asa_exit(): curvature_flag");
    Exit_ASA (exit_msg);
    *exit_status = CALLOC_FAILED;
    return (9);
  }
...
#if MULTI_MIN
  if (MULTI_NUMBER <= 0) {
    strcpy (exit_msg, "*** MULTI_NUMBER <= 0 ***");
    print_string (ptr_asa_out, exit_msg);
    ++invalid;
  }
  VFOR (index_v) {
...
#endif /* HAVE_ANSI */
{
  ...
  *j = (int) jj;
  ...
  if (multi_cost[*i] > multi_cost[*j] + (double) EPS_DOUBLE)
    return (1);
  else if (multi_cost[*i] < multi_cost[*j] - (double) EPS_DOUBLE)
    return (-1);
  else
    return (0);
}
...
sort_parallel (const void *ii, const void *jj)
{
  ...
  if (gener_block_state[*i].cost > gener_block_state[*j].cost)
    return (1);
  else if (gener_block_state[*i].cost < gener_block_state[*j].cost)
    return (-1);
  else
    return (0);
}
